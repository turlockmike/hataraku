...
export class McpClient {
  ...
  async callTool(serverName: string, toolName: string, args: Record<string, unknown>) {
    ...
    const availableServers = this.connections.filter(conn => !conn.server.disabled).map(conn => conn.server.name)
    ...
    if (!toolName) {
      throw new Error(
        `Tool name is required. Available tools for ${serverName}: ${availableTools.join(', ') || 'None'}`,
      )
    }

    if (availableTools.length > 0 && !availableTools.includes(toolName)) {
      throw new Error(
        `Tool "${toolName}" not found in server "${serverName}". Available tools: ${availableTools.join(', ')}`,
      )
    }
