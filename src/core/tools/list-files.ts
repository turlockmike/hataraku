...
export const listFilesTool: Tool = {
  description:
    'List files and directories within the specified directory. If recursive is true, it lists all files (with relative paths) recursively.',
  parameters: z.object({
    path: z
      .string()
    ...
    recursive: z
      .boolean()
      .optional()
      .describe('Set to true for a recursive listing, false or omitted for top-level only.'),
  }),
  /**
  ...
  execute: async ({ path: dirPath, recursive = false }) => {
    try {
      ...
      const message = relativePaths.length > 0 ? relativePaths.sort().join('\n') : '(empty directory)'
