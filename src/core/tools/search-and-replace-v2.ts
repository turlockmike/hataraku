import { addLineNumbers, everyLineHasLineNumbers, stripLineNumbers } from '../misc/extract-text'
...
<end_line>5</end_line>
  ...
  async applyDiff(originalContent: string, diffContent: string, startLine?: number, endLine?: number) {
    ...
    if (matchIndex === -1 || bestMatchScore < this.fuzzyThreshold) {
      ...
      return {
        ...
        error: `No sufficiently similar match found${lineRange} (${Math.floor(
          bestMatchScore * 100,
        )}% similar, needs ${Math.floor(this.fuzzyThreshold * 100)}%)\n\nDebug Info:\n- Similarity Score: ${Math.floor(
          bestMatchScore * 100,
...
export const searchReplaceV2Tool: Tool = {
  description: new SearchReplaceDiffStrategy().getToolDescription(),
  parameters: z.object({
    path: z.string().describe('The path of the file to modify (relative to the current working directory)'),
    diff: z
      .string()
      .describe(
        'The search/replace block defining the changes in the format: <<<<<<< SEARCH\\n[content]\\n=======\\n[content]\\n>>>>>>> REPLACE',
      ),
    start_line: z.number().optional().describe('The line number where the search block starts (1-based)'),
    end_line: z.number().optional().describe('The line number where the search block ends (1-based)'),
    fuzzy_threshold: z
      .number()
      .optional()
      .describe('Similarity threshold for fuzzy matching (0.0-1.0, where 1.0 is exact match)'),
  }),
  execute: async ({ path: filePath, diff, start_line, end_line, fuzzy_threshold }) => {
    try {
      ...
      // Create strategy with optional fuzzy threshold
      const strategy = new SearchReplaceDiffStrategy(fuzzy_threshold)

      // Apply the diff
      const result = await strategy.applyDiff(fileContent, diff, start_line, end_line)
      ...
      if (!result.success) {
        return {
          ...
          content: [
            {
              type: 'text',
              text: result.error || 'Failed to apply diff',
            },
          ],
        }
      }
