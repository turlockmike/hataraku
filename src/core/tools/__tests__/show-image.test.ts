...
describe('showImageTool', () => {
  ...
  it('should display PNG image on Windows', async () => {
    ...
    expect(mockChildProcess.exec).toHaveBeenCalledWith('start "" "/mock/path/test.png"', expect.any(Function))
  })
  ...
  it('should display JPEG image on macOS', async () => {
    ...
    expect(mockChildProcess.exec).toHaveBeenCalledWith('open "/mock/path/test.jpg"', expect.any(Function))
  })
  ...
  it('should display image on Linux', async () => {
    ...
    expect(result.isError).toBeUndefined()
    expect(result.content[0].text).toBe('Displaying image: test.png')
    expect(mockChildProcess.exec).toHaveBeenCalledWith('xdg-open "/mock/path/test.png"', expect.any(Function))
  })
  ...
  it('should support all documented formats', async () => {
    ...
    for (const format of supportedFormats) {
      ...
      expect(mockChildProcess.exec).toHaveBeenCalledWith(`open "/mock/path/test${format}"`, expect.any(Function))
    }
  })
})
