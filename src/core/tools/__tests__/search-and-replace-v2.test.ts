import { SearchReplaceDiffStrategy } from "../search-and-replace-v2"
...
function two() {
	...
	describe("getToolDescription", () => {
		...
		it("should include the current working directory", async () => {
			const cwd = "/test/dir"
			const description = await strategy.getToolDescription()
			expect(description).toContain(`relative to the current working directory`)
		})
...
describe("searchReplaceV2Tool", () => {
	...
	beforeEach(() => {
		jest.clearAllMocks()
		path.resolve.mockImplementation((cwd, filePath) => `/mock/path/${filePath}`)
	...
	it('should successfully apply diff to file', async () => {
		const originalContent = 'function test() {\n    return true;\n}\n'
		const diff = `test.ts\n<<<<<<< SEARCH\nfunction test() {\n    return true;\n}\n=======\nfunction test() {\n    return false;\n}\n>>>>>>> REPLACE`

		// Make sure access doesn't reject for this test
		fs.access.mockResolvedValue(undefined)
		fs.readFile.mockResolvedValue(originalContent)
		fs.writeFile.mockResolvedValue(undefined)

		const result = await searchReplaceV2Tool.execute({
			path: 'test.ts',
			diff,
			start_line: 1,
			end_line: 3
		...
		expect(result.content[0].text).toBe('Successfully applied diff to test.ts')
	...
	it('should handle file not found error', async () => {
		...
		expect(result.content[0].text).toBe('File not found at path: nonexistent.ts')
	})
	...
	it('should handle invalid diff format', async () => {
		...
		expect(result.isError).toBe(true)
		expect(result.content[0].text).toContain('Invalid diff format - missing required SEARCH/REPLACE sections')
	})

	it('should handle fuzzy matching with threshold', async () => {
		const originalContent = 'function test() {\n    return true;\n}\n'
		const diff = `test.ts\n<<<<<<< SEARCH\nfunction test(){\n  return true;\n}\n=======\nfunction test() {\n    return false;\n}\n>>>>>>> REPLACE`
	...
	it('should handle write file errors', async () => {
		...
		expect(result.isError).toBe(true)
		expect(result.content[0].text).toBe('Error applying diff: Write error')
	})
})