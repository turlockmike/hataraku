...
export async function createKnowledgeBaseProvider(
  ...
  if (!effectiveConfig.knowledgeBaseId) {
    throw new Error('Knowledge Base ID is required. Please provide it via CLI (--kb-id), environment variable (KB_ID), or in your profile configuration.');
  }
    ...
    try {
      ...
      const sources = response.citations?.flatMap(citation => 
        citation.retrievedReferences?.map(reference => {
          ...
          let url = reference.location?.type === 'WEB' 
