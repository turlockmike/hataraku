...
const initializeTasks = async () => {
  ...
  const addNumbersTask = createTask({
    ...
    task: input => `Use the add tool to add these numbers: ${input.a} and ${input.b}`,
  })
  ...
  const addNumbers: TaskExecutor<z.infer<typeof mathInputSchema>, number> = async input => {
  ...
  const multiplyNumbers: TaskExecutor<z.infer<typeof mathInputSchema>, number> = async input => {
  ...
  const numbersToWords: TaskExecutor<z.infer<typeof numbersToWordsSchema>, string> = async input => {
...
async function main() {
  ...
  const mathWorkflow = createWorkflow<{ pairs: [number, number][] }>(
    ...
    async w => {
      ...
      const finalProduct = await w.task('Multiply Results', tasks.multiplyNumbers, { a: firstSum, b: secondSum })

      console.log(chalk.cyan('\n📊 Converting to words...'))

      // Convert to words
      const inWords = await w.task('In Words', tasks.numbersToWords, { number: finalProduct })
